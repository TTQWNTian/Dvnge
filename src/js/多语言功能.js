/* TTQ视觉小说引擎v1.8.0-dev */
/* js/多语言功能.js */
export let 当前语言 = localStorage.getItem('Dvnge_语言') || null;
export let 语言配置表 = null;

export function 加载语言配置() {
...
export function 切换语言(新语言代码) {
    ...
    if (window.当前状态 && window.当前状态.逐字显示?.打字定时器) {
        clearTimeout(window.当前状态.逐字显示.打字定时器);
        window.当前状态.逐字显示.打字定时器 = null;
    }
    const 内容元素 = document.querySelector('.内容');
    if (内容元素) 内容元素.dataset.正在打字 = 'false';
    
    刷新界面文字();
    if (window.刷新当前对话) window.刷新当前对话();
}

export function 刷新界面文字() {
    if (!语言配置表) return;
...
export function 获取界面文字(标识) {
    if (!语言配置表 || !当前语言) return 标识;
    
    const 默认语言 = 语言配置表.默认语言;
    if (!默认语言) return 标识;
    
    if (当前语言 === 默认语言) return 标识;
    
    const 文字项 = 语言配置表.界面文字?.[标识];
    if (!文字项) return 标识;
    
    return 文字项[当前语言] || 文字项[默认语言] || 标识;
}

export function 获取本地化文本(节点) {
    if (!节点) return { 角色: '', 内容: '' };
    ...
    return {
        角色: 节点[`角色_${当前语言}`] || 节点[`角色_${语言配置表.默认语言}`] || 节点.角色 || '',
        内容: 节点[`内容_${当前语言}`] || 节点[`内容_${语言配置表.默认语言}`] || 节点.内容 || ''
    };
}

export function 刷新当前对话() {
    if (!window.章节库 || !window.当前状态) return;
    
    const 当前章节数据 = window.章节库[window.当前状态.当前章节];
    if (!当前章节数据) return;
    
    const 当前节点 = 当前章节数据[window.当前状态.当前索引];
    if (!当前节点) return;
    ...
    // 选项和输入框的刷新逻辑（保持原样，但需要把 语言配置表.默认语言 的引用改成通过模块）
...
window.切换语言 = 切换语言;
window.刷新界面文字 = 刷新界面文字;
window.获取界面文字 = 获取界面文字;
window.获取本地化文本 = 获取本地化文本;
window.刷新当前对话 = 刷新当前对话;