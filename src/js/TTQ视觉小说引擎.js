...
const 章节文件列表 = [
    { 名称: "序章", 路径: "chapter/序章.js" },
    // 后续添加更多章节：{ 名称: "第一章", 路径: "chapter/第一章.js" },
];
...
function 获取本地化文本(节点) {
    ...
    const 角色字段 = `角色_${当前语言}`;
    const 内容字段 = `内容_${当前语言}`;
    const 默认角色字段 = `角色_${语言配置表.默认语言}`;
    const 默认内容字段 = `内容_${语言配置表.默认语言}`;
...
function 清除所有动画() {
    ...
    const 角色元素 = document.querySelector('.角色');
    if (角色元素) {
        角色元素.style.animation = '';
    }
    
    const 内容元素 = document.querySelector('.内容');
    if (内容元素) {
        内容元素.style.animation = '';
    }
    
    const 背景容器 = document.getElementById('背景容器');
    if (背景容器) {
        背景容器.style.animation = '';
    }
...
function 更新场景(当前节点) {
    ...
    if (节点.动画?.左立绘) {
        应用动画(document.getElementById('左立绘'), 节点.动画.左立绘);
    }
    if (节点.动画?.中立绘) {
        应用动画(document.getElementById('中立绘'), 节点.动画.中立绘);
    }
    if (节点.动画?.右立绘) {
        应用动画(document.getElementById('右立绘'), 节点.动画.右立绘);
    }
...
function 更新存档显示(存档位) {
    ...
    if (数据) {
        ...
        const 存档文字 = 是自动存档 ? 获取界面文字('自动存档') : `${获取界面文字('手动存档')}${存档位}`;
        存档项.innerHTML = `<span>${存档文字}: ${章节显示} · ${时间显示}</span>
     <button class="加载按钮">${获取界面文字('加载')}</button>`;
    } else {
        const 空存档文字 = 是自动存档 ? 获取界面文字('自动存档') : `${获取界面文字('手动存档')}${存档位}`;
        存档项.innerHTML = `<span>${空存档文字}: ${获取界面文字('空存档位')}</span>`;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        console.warn('语言配置加载失败，使用原始内容:', e);
        语言配置表 = null;
        当前语言 = null;
    }
