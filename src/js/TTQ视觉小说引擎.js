...
function 解析指令行(行) {
    ...
    switch (指令名) {
        ...
        case '立绘媒体': {
            ...
            结果.立绘[位置].媒体 = {
                循环: 参数.循环 !== 'false',
                播放次数: parseInt(参数.播放次数),
                播放间隔: parseFloat(参数.播放间隔),
                音量: parseFloat(参数.音量)
            };
            break;
        }
        ...
        case '设置变量': {
            const 设置 = {};
            解析设置变量(设置, 匹配[2]);
            if (Object.keys(设置).length > 0) 结果.设置变量 = 设置;
            break;
        }
        ...
        case '读档时设置变量': {
            const 设置 = {};
            解析设置变量(设置, 匹配[2]);
            if (Object.keys(设置).length > 0) 结果.读档时设置变量 = 设置;
            break;
        }
        ...
        case '逐字显示':
            if (参数[0] === 'false') {
                ...
                结果.逐字显示 = { 启用: true, 速度: parseFloat(参数[0]) };
            }
            break;
    ...
    for (const 块 of 块列表) {
        ...
        for (let i = 0; i < 行列表.length; i++) {
            ...
            if (当前调查节点 && 行.startsWith('区域 ')) {
                ...
                当前调查节点.调查.区域.push({
                    x: parseInt(区域参数.x) || parseInt(区域参数[0]),
...
// ====================== 变量设置工具 ======================
function 安全获取变量(变量路径, 默认值 = '') {
    if (!变量路径) return 默认值;
    const 路径段 = 变量路径.trim().split('.');
    let 值 = 当前状态?.用户变量;
    for (let 段 of 路径段) {
        if (值 === undefined || 值 === null) break;
        值 = 值[段];
    }
    return (值 !== undefined && 值 !== null) ? 值 : 默认值;
}

function 替换变量(文本, 默认值 = '') {
    if (!文本) return 文本;
    return 文本.replace(/{([^}]+)}/g, (匹配, 变量路径) => {
        return 安全获取变量(变量路径, 默认值);
    });
}
...
function 解析变量设置值(值) {
    if (typeof 值 === 'string') {
        ...
        if (原始值.startsWith('=')) {
            原始值 = 原始值.slice(1).trim();
        }
        
        const 操作符匹配 = 原始值.match(/^(\+|\-|\*|\/)=(-?\d+\.?\d*)/);
        if (操作符匹配) {
            return {
                操作符: 操作符匹配[1],
                数值: parseFloat(操作符匹配[2])
            };
        }
        
        const 数字值 = parseFloat(原始值);
        if (!isNaN(数字值) && 原始值.trim() !== '') {
            return 数字值;
        }
...
function 解析设置变量(设置变量对象, 值字符串) {
    if (!值字符串) return;
    值字符串.split(',').forEach(v => {
        const 匹配 = v.match(/^"([^"]+)"\s*(.+)$/);
        if (匹配) {
            const 变量名 = 匹配[1].trim();
            const 值 = 匹配[2].trim();
            设置变量对象[变量名] = 解析变量设置值(值);
        }
    });
}
...
function 开始逐字显示(内容元素, 完整文本, 速度, 当前节点) {
    ...
    if (当前状态.打字音效 && 当前状态.打字音效.启用 && 当前状态.打字音效.路径) {
        try {
            const 现有音效 = 当前状态.打字音效.当前音效对象;
            if (现有音效 && 现有音效.src && 现有音效.src.includes(当前状态.打字音效.路径)) {
                打字音效对象 = 现有音效;
                打字音效对象.currentTime = 0;
            } else {
                打字音效对象 = new Audio(当前状态.打字音效.路径);
                打字音效对象.volume = Math.min(1, Math.max(0, 当前状态.打字音效.音量 || 0.5));
                打字音效对象.load();
                当前状态.打字音效.当前音效对象 = 打字音效对象;
            }
        } catch (e) {
            打字音效对象 = null;
    ...
    const 标签堆栈 = [];
    let 字符计数 = 1;
    const 音效启用 = !!(当前状态.打字音效 && 当前状态.打字音效.启用 && 打字音效对象);
    const 音效间隔 = 当前状态.打字音效.间隔字符数;
...
function 停止打字效果() {
    ...
    if (当前状态.打字音效 && 当前状态.打字音效.当前音效对象) {
        try {
            const 音效 = 当前状态.打字音效.当前音效对象;
            if (音效 && typeof 音效.pause === 'function') {
                音效.pause();
                音效.currentTime = 0;
            }
        } catch (e) {
...
function 更新场景(当前节点) {
    ...
    if (容器) {
        ...
        if (!有对话内容 && !有选项 && !有输入框) {
            ...
            let 处理后的内容 = 替换变量(本地化内容, '未知');
            
            const 节点逐字配置 = 节点.逐字显示 || {};
            const 逐字启用 = 节点逐字配置.启用 !== undefined ? 节点逐字配置.启用 : 当前状态.逐字显示.启用;
            
            const 初始速度 = 读取配置('初始状态').逐字显示.速度;
            const 逐字速度 = 节点逐字配置.速度 !== undefined ? 节点逐字配置.速度 : 初始速度;
